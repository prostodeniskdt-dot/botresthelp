...
Файл `DATA_DIR/allowed_users.json` (в репозиторий **не коммитится**). При первом запуске, если файла нет, он копируется из `data/allowed_users.example.json` (рядом с примером в репозитории).
...
В репозитории уже есть `.gitignore` для `data/allowed_users.json` и `data/sessions.json`.
