...
def _set_category_back(session: dict[str, Any], category_id: str, page: int) -> None:
    session["ttk_back"] = f"tk:c:{category_id}:{page}"
...
async def _send_card(
    ...
    markup = ttk_card_keyboard(item, back_callback=back_cb)
...
async def on_ttk_category(callback: CallbackQuery, session: dict[str, Any]) -> None:
    ...
    if not items:
        logger.warning("ttk_empty_category category=%s", category_id)
        ...
        await callback.answer()
        return
    _set_category_back(session, category_id, page)
    title = store.category_title(category_id)
    await callback.message.edit_text(
        f"{title}. Выберите позицию:",
        reply_markup=ttk_items_keyboard(store, category_id, page),
    )
    await callback.answer()
...
async def on_ttk_item(callback: CallbackQuery, session: dict[str, Any]) -> None:
    ...
    if not item or item.get("archived"):
        logger.error("ttk_item_not_found id=%s", item_id)
        await callback.answer("Позиция не найдена", show_alert=True)
        return
    if not session.get("ttk_back"):
        cat_id = str(item.get("category_id") or "")
        _set_category_back(session, cat_id, 0)
...
async def on_ttk_item_view(callback: CallbackQuery, session: dict[str, Any]) -> None:
    ...
    item = store.items_by_id.get(item_id)
    if not item or item.get("archived"):
        logger.error("ttk_item_not_found id=%s view=%s", item_id, view)
        await callback.answer("Позиция не найдена", show_alert=True)
        return
...
async def on_ttk_search_page(callback: CallbackQuery, session: dict[str, Any]) -> None:
    ...
    await callback.message.edit_text(
        ...
        reply_markup=ttk_search_results_keyboard(results, page, page_size=TTK_PAGE_SIZE),
    )
...
async def handle_ttk_search(message: Message, session: dict[str, Any], query: str) -> bool:
    ...
    if len(results) == 1:
        await _send_card(message, results[0], "d", session)
        session["flow"] = "idle"
        return True
    await message.answer(
        f"Нашёл {len(results)} позиций — выберите:",
        reply_markup=ttk_search_results_keyboard(results, 0, page_size=TTK_PAGE_SIZE),
    )
    session["flow"] = "idle"
    return True
